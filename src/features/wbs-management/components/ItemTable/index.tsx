...
import { ChevronDown, ChevronUp, Edit, Filter, Save, X } from "lucide-react";
...
export const ItemTable: React.FC<ItemTableProps> = ({
	...
	const { columnDefs } = useItemTable(data);
	...
	const handleCellChange = useCallback(
		(rowIndex: number, columnId: keyof Item, value: string | number) => {
			setEditingData((prevData) => {
				...
				if (row?.original) {
					...
					const actualRowIndex = newData.findIndex(
						(item) =>
			...
			<div className="flex-shrink-0 flex items-center justify-between p-4 bg-white border-b border-gray-200 shadow-sm">
				...
				<div className="flex items-center gap-2">
					...
					<Button
						...
						title={
							isEditMode
								? "Filters are always visible in edit mode"
				...
				<table className="w-full">
								...
								return (
									<th
										...
										style={{
											...
											minWidth: columnDefs.find((c) => c.id === column.id)
												?.minSize,
											maxWidth: columnDefs.find((c) => c.id === column.id)
												?.maxSize,
										}}
									>
										<button
					...
					<tbody>
						...
						{paddingTop > 0 && (
							<tr>
								<td
									colSpan={tableWithEditData.table.getAllColumns().length}
